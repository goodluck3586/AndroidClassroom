...
public class MainActivity extends AppCompatActivity implements View.OnClickListener{
    ...
    public void onClick(View v) {
        ...
        final int checkedIndex = listView.getCheckedItemPosition();       // 리스트뷰에서 체크된 아이템의 Index

        switch (v.getId()){
            case R.id.btnAdd:
                int count = adapter.getCount();                 // 어댑터에 연결되어 있는 아이템의 개수 저장
                dataList.add("리스트 데이터 " + (count+1));     // ArrayList에 데이터 추가
                break;
            case R.id.btnModify:
        ...
        listView.clearChoices();                        // 체크상태 초기화
    }
}
